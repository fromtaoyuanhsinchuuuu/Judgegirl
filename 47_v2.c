...
int totalNum(Node *root)
{
    if (root == NULL) return 0;
    return root->num + root->left_child_num + root->right_child_num;
}
...
Node *insertNode(char c, int num, int index, Node *root)
{
    ...
    if (index >= root->left_child_num + 1 && index <= root->left_child_num + root->num){
        ...
        if (left_num > 0){
            root->left = insertNode(root->c, left_num, totalNum(root->left) + 1, root->left);
            root->left_child_num += left_num;
        }
        int right_num = root->num - left_num;
        if (right_num > 0){
            root->right = insertNode(root->c, right_num, 1, root->right);
